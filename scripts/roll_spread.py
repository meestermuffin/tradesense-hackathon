...
"""
import argparse, csv, datetime, math, os, random, statistics as st, sys
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from src.data.alpaca import AlpacaClient, DATA_HOST
...
def perm_p(a, b, draws=200000, seed=42):
    ...
    obs = spearman(a, b); rng = random.Random(seed); hits = 0
    bb = list(b)
    for _ in range(draws):
        rng.shuffle(bb)
        if spearman(a, bb) >= obs: hits += 1
...
def main():
    ...
    day = a.trade_date or (datetime.date.fromisoformat(
        os.path.basename(nbbo)[5:15]) - datetime.timedelta(days=1)).isoformat()
    ...
    for r in rows:
        try:
            d = c.request("GET", DATA_HOST, "/v1beta1/options/trades",
        ...
        out.append(dict(day=day, symbol=r["symbol"], underlying=r["underlying"],
                        tenor=r["tenor"], strike=r["strike"], type=r["type"],
                        n_trades=len(tr), roll=("" if est is None else round(est, 4)),
                        drop_reason=why or "",
                        measured_spread=r["spread"], measured_spread_pct=r["spread_pct"],
                        mid=r["mid"]))

    od = os.path.join(ROOT, "data", "roll"); os.makedirs(od, exist_ok=True)
    ...
    drops = {}
    for o in out:
        if o["drop_reason"]: drops[o["drop_reason"]] = drops.get(o["drop_reason"], 0) + 1
    for k, v in sorted(drops.items()): print(f"  dropped {v:3}  {k}")
    ...
    print(f"  p is reported alongside rho deliberately: n=11 has no usable asymptotics, and the")
    print(f"  review computed p ~ 0.061 for rho +0.500 — a lead, not a settled result.")
