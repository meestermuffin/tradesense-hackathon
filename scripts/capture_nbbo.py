...
def main():
    ...
    for expiry, tenor in next_expiries(today):
        for s in UNIVERSE:
            ...
            try:
                ...
                continue
            near = sorted(cs, key=lambda x: abs(float(x["strike_price"]) - spot[s]))[:STRIKES_PER_TENOR]
            for k in near:
                syms.append(k["symbol"])
                meta[k["symbol"]] = dict(underlying=s, expiry=expiry, tenor=tenor,
                                         strike=float(k["strike_price"]), type=k["type"],
                                         spot=spot[s])
    if not syms:
    ...
    for sym, q in quotes.items():
        ...
        m = meta[sym]
        mid = (bid + ask) / 2
        rows.append(dict(captured_at=captured_at, rth=is_open, symbol=sym, **m,
                         bid=bid, ask=ask, bid_sz=q.get("bs"), ask_sz=q.get("as"),
                         mid=round(mid, 4), spread=round(ask - bid, 4),
                         spread_pct=round((ask - bid) / mid, 6),
                         moneyness=round(m["strike"] / m["spot"], 4), quote_t=q.get("t")))
    if not rows:
    ...
    print(f"captured {len(rows)} quotes across {len({r['underlying'] for r in rows})} names "
