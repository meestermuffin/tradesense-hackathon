...
def build(src):
    ...
    for sym in src.symbols():
        ...
        days = [r["day"] for r in rows]; iv = [float(r["iv"]) for r in rows]
        spot = [float(r["spot"]) for r in rows]; rec = []
        for i in range(len(rows)):
            if i + H >= len(rows) or i < max(PCT_MIN_OBS, TRAIL_RV): continue
            fwd = realized_vol(spot[i:i + H + 1]); trail = realized_vol(spot[i - TRAIL_RV:i + 1])
            if fwd is None or trail is None or trail <= 0: continue
            win = iv[max(0, i - PCT_WINDOW):i]
            if len(win) < PCT_MIN_OBS: continue
            pct = 100.0 * sum(1 for v in win if v <= iv[i]) / len(win)
            rec.append(dict(day=days[i], i=i, A=pct, B=iv[i] / trail, C=iv[i],
                            out=(iv[i] - fwd) / iv[i]))
        per[sym] = (days, rec)
    return per
...
def forward_contaminated(per, earn):
    ...
    for sym, (days, _) in per.items():
        ...
        for i in range(len(days)):
            ...
            if any(lo <= d <= hi for d in ds): bad.add(days[i])
        out[sym] = bad
    return out
...
def blackout_days(per, earn, drop_delivery):
    ...
    out = {}
    for sym, (days, _) in per.items():
        dates = [d for d in earn.get(sym, []) if not (drop_delivery and sym == "TSLA" and d in TSLA_DELIVERY)]
        idx = set()
        for d in dates:
            ...
            for j in range(k - BLACKOUT, k + BLACKOUT + 1):
                if 0 <= j < len(days): idx.add(days[j])
        out[sym] = idx
    return out
...
def report(label, per, excl):
    ...
    dropped = sum(len([x for x in r if x['day'] in excl.get(s, set())]) for s, (_, r) in per.items()) if excl else 0
    print(f"    name-days {tot - dropped} of {tot} ({dropped} excluded)   days {n_days}")
    res = {}
    for k, lab in (("A", "A · IV percentile"), ("B", "B · IV/trailing RV [confounded]"),
                   ("C", "C · raw IV [control]")):
        ds = daily(per, k, excl)
        ics = [spearman(s, o) for s, o in ds]; ics = [x for x in ics if x is not None]
        ic = sum(ics) / len(ics); t = newey_west_t(ics, H)
        _, p = permutation_p(ds, mean_ic, draws=DRAWS, seed=SEED)
        res[k] = ic
        print(f"    {lab:34} IC {ic:+.4f}   NW t {t if t is None else round(t,2):>6}   perm p {p:.4f}")
    return res

def main():
    src = FileFeatureSource(DATA); per = build(src)
    earn = json.load(open(EARN))
    base = report("ALL name-days (baseline)", per, None)
    prim = report("scheduled-event days REMOVED (primary: all Item 2.02)",
                  per, blackout_days(per, earn, drop_delivery=False))
    sens = report("sensitivity: TSLA delivery dates NOT treated as events",
                  per, blackout_days(per, earn, drop_delivery=True))
    print(f"\n{'='*70}\nJOB 2 — verdict rests on A, read against control C")
    print(f"  A: baseline {base['A']:+.4f}  ->  events removed {prim['A']:+.4f} "
          f"(change {prim['A']-base['A']:+.4f})")
    print(f"  C: baseline {base['C']:+.4f}  ->  events removed {prim['C']:+.4f}")
    print(f"  sensitivity A (delivery kept in sample): {sens['A']:+.4f} "
          f"(vs primary {prim['A']:+.4f}, diff {sens['A']-prim['A']:+.4f})")
    pw = report("POWERED: forward outcome window contains NO announcement",
                per, forward_contaminated(per, earn))
    print(f"  POWERED A: {pw['A']:+.4f}  (baseline {base['A']:+.4f}, change {pw['A']-base['A']:+.4f})")
    ch = prim['A'] - base['A']
