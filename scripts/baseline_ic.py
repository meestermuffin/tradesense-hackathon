...
DATA = os.path.join(
    ...
    "data",
    "iv_series_2024-03_2025-02.csv.gz",
)
...
def build(src):
    ...
    for sym in src.symbols():
        ...
        for i in range(len(rows)):
            ...
            rec.append(
                dict(day=days[i], A=pct, B=iv[i] / trail, C=iv[i], out=(iv[i] - fwd) / iv[i])
            )  # return on premium sold (addendum)
        per[sym] = rec
    return per
...
def main():
    ...
    res, ics_by = {}, {}
    for key, label in (
        ("A", "A · IV percentile (126/63 trailing)"),
    ...
    if icc >= best:
        print(
            "  -> C MATCHES OR EXCEEDS A/B: the percentile machinery adds nothing over "
            "selling whatever has the highest IV. Signal as designed NOT JUSTIFIED."
        )
    elif best - icc < 0.25 * abs(best) if best else False:
        print(
            "  -> A/B exceed C only marginally: ranking against own history is doing little work."
        )
    else:
