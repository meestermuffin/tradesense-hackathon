...
def main():
    ...
    a = c.account()
    pos = c.positions()
    row = dict(day=datetime.date.today().isoformat(),
               captured_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),
               account=a["account_number"], equity=a["equity"], last_equity=a.get("last_equity"),
               cash=a["cash"], position_market_value=a.get("position_market_value"),
               open_legs=len(pos),
    ...
    with open(OUT, "a", newline="") as fh:
        ...
        if new:
            w.writeheader()
        w.writerow(row)
    print(f"{row['day']}  equity {row['equity']}  open legs {row['open_legs']}  -> "
