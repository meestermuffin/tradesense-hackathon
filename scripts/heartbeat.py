...
def main():
    ...
    if os.path.exists(cap):
        ...
        problems.append(f"NO NBBO CAPTURE for {iso}. That session's spreads are gone permanently — "
    ...
    if os.path.exists(eq) and any(l.startswith(iso) for l in open(eq)):
        ...
        problems.append(f"NO EQUITY ROW for {iso}. Sharpe and max drawdown are computed from "
    ...
    if os.path.exists(log):
        ...
        if age_h > 24:
            problems.append(f"cycle log has not been written in {age_h:.0f}h — the scheduled cycle "
                            f"is not running")
    else:
    ...
    if problems:
        ...
        if "--notify" in sys.argv:
            ...
            os.system("osascript -e " + chr(39) + "display notification " + chr(34) + head
                      + chr(34) + " with title " + chr(34) + "tradesense: "
                      + str(len(problems)) + " problem(s)" + chr(34) + chr(39)
                      + " >/dev/null 2>&1")
        return 1
    print("\nall three ran.")
    return 0
